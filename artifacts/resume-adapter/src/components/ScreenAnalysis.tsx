      ...
      <Card className="bg-white shadow-sm border-blue-100">
        <CardContent className="p-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div>
              <h2 className="text-xl font-bold text-slate-900">Соответствие вакансии</h2>
              <p className="text-slate-500 text-sm mt-1">Опишите недостающий опыт, чтобы улучшить резюме</p>
            </div>
        ...
        {missingReqs.map((req, i) => (
          ...
          >
            <Card className="border-red-200 bg-red-50/30 overflow-hidden shadow-sm">
              ...
              <CardContent>
                <Textarea
                  placeholder="Есть ли у вас такой опыт? Опишите кратко..."
            ...
            <Card className="border-yellow-200 bg-yellow-50/30 shadow-sm">
              <CardContent className="p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex-1">
                  <p className="font-medium text-slate-900 text-sm">{req.text}</p>
        ...
        {confirmedReqs.map((req, i) => (
          ...
          >
            <Card className="border-green-200 bg-green-50/30 shadow-sm">
              <CardContent className="p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <p className="font-medium text-slate-900 text-sm flex-1">{req.text}</p>
                <div className="flex gap-2 shrink-0">
                  <Badge variant={req.criticality === "must" ? "default" : "secondary"} className={req.criticality === "must" ? "bg-blue-600 hover:bg-blue-700" : ""}>
                    {req.criticality === "must" ? "обязательное" : "желательное"}
                  </Badge>
